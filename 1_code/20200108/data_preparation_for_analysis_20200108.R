...
setwd(
  "3_data_analysis/data_analysis20200108/urine_metabolome/data_cleaning/RPLC/POS/"
)
load("rplc_pos_6")

setwd(r4projects::get_project_wd())
setwd(
  "3_data_analysis/data_analysis20200108/urine_metabolome/data_cleaning/RPLC/NEG/"
)
load("rplc_neg_6")
...
cor_value_pos <-
  cor_value %>%
  filter(stringr::str_detect(name1, "_POS") &
           stringr::str_detect(name2, "_POS"))

cor_value_neg <-
  cor_value %>%
  filter(stringr::str_detect(name1, "_NEG") &
           stringr::str_detect(name2, "_NEG"))
...
cyto_id <-
  match(cytokine_data$Name, urine_id$sample_id_HIMC) %>%
