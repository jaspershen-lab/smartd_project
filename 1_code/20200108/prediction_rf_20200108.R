...
predicted_y <-
  predict(object = rf_regression, newdata = sample_data_val_x_rf
          ...
          ggsave("discovery_data_measured_vs_predicted.pdf",
                 width = 7,
                 height = 7)
          ...
          ggsave("measured_vs_predicted_error.pdf",
                 width = 7,
                 height = 7)
          ...
          ggsave("discovery_data_measured_vs_predicted2.pdf",
                 width = 7,
                 height = 7)
          ...
          ggsave("measured_vs_predicted_val.pdf",
                 width = 7,
                 height = 7)
          ...
          for (i in 1:100) {
            ...
            dis_index <-
              sample(
                1:nrow(sample_data_dis_x_rf),
                size = nrow(sample_data_dis_x_rf),
                replace = TRUE
              ) %>%
              unique() %>%
              sort()
          ...
          ggsave("measured_vs_predicted_dis.pdf",
                 width = 7,
                 height = 7)
          ...
          date_info_dis <-
            sample_data_dis[, c("Patient_ID",
                                ...
                                "EDD")] %>%
            mutate(
              begin_date = as.Date(day_0, "%m/%d/%Y"),
          ...
          predicted_y <-
            predict(object = rf_regression, newdata = sample_data_val_x_rf
                    ...
                    ###from here we can see that why should us linear regression to correct prediction
                    data.frame(
                      measured = expected_date_remained_dis[, 1],
                      predicted = prediction_self,
                      stringsAsFactors = FALSE
                    ) %>%
                      mutate(diff = predicted - measured) %>%
