...
rm(list = ls())
setwd("3_data_analysis/data_analysis20200108/prediction/metabolites/")

load("sample_data_dis")
load("sample_data_val")

setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_analysis20200108/patient_information")
library(tidyverse)
sample_info <- readr::read_csv("sample_info_191021.csv")

temp_data <-
  data.frame(
    subject_id = sample_info$Patient_ID,
...
temp_data <-
  ...
  mutate(
    batch =
      case_when(
        subject_id %in% sample_data_dis$Patient_ID ~ 1,
...
sample_information <-
  temp_data %>%
  dplyr::left_join(sample_info[, c(2, 5)], by = c("sample_id" = "Sample_ID"))

xlsx::write.xlsx(sample_information, "sample_information.xlsx")
...
temp_data2$prediction <-
  factor(temp_data2$prediction,
         levels = c("Validation", "Discovery", "NA"))
...
alluvial(
  ...
  col = ifelse(tit$Class == "3rd" &
                 tit$Sex == "Male", "red", "gray")
)
