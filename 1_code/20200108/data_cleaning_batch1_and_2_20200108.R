...
setwd(
  "2_data/data_analysis20200108/urine_metabolome/data_cleaning/RPLC/POS/"
)
...
for (i in seq_along(name)) {
  temp_date <- stringr::str_extract(string = name[i], pattern = "_[0-9]{6,15}") %>%
  ...
  temp_peak_table <-
    peak_table[, c(which(paste(colnames(peak_table), temp_date, sep = "_") == name[i]), which(colnames(peak_table) == name[i]))]
  
  
  remove_name <- c(
    remove_name,
    apply(temp_peak_table, 2, function(x)
      sum(is.na(x))) %>%
      which.min() %>%
      names() %>%
...
(
  rplc_pos_2 <-
    filterPeak(
      object = rplc_pos_1,
      min.fraction.qc = 0.8,
      ##QC at least more than 80%
      min.fraction = 0.2,
      ##Subject no restraction
      min.subject.qc.ratio = 0,
      ##don't use blank to remove any peaks
      dl.qc.r2.cutoff = 0
    )
)
...
(rplc_pos_3 <-
    filterSample(object = rplc_pos_2, min.fraction.peak = 0.5))
...
(
  plot1 <-
    ...
    scale_y_continuous(
      ...
      limits = c(0, 100),
      breaks = c(0, 25, 30, 50, 75, 100),
      labels = c(c(0, 25, 30, 50, 75, 100))
    ) +
    labs(x = "Peak index") +
    theme_bw() +
    theme(
...
(rplc_pos_5 <-
    normalizeData(object = rplc_pos_4, method = "mean"))

save(rplc_pos_5, file = "rplc_pos_5")
###batch intergration

(rplc_pos_6 <-
    metflow2:::integrateData(object = rplc_pos_5, method = "subject.mean"))

qc_rsd <- calRSD(object = rplc_pos_6, slot = "QC")
...
###remove some samples (outliers)
(rplc_neg_3 <-
    filterSample(object = rplc_neg_2, min.fraction.peak = 0.5))
...
(rplc_neg_5 <-
    normalizeData(object = rplc_neg_4, method = "mean"))

save(rplc_neg_5, file = "rplc_neg_5")
###batch intergration

(rplc_neg_6 <-
    metflow2:::integrateData(object = rplc_neg_5, method = "subject.mean"))

qc_rsd <- calRSD(object = rplc_neg_6, slot = "QC")
...
###remove some samples (outliers)
(hilic_pos_3 <-
    filterSample(object = hilic_pos_2, min.fraction.peak = 0.5))
...
(hilic_pos_5 <-
    normalizeData(object = hilic_pos_4, method = "mean"))
...
(
  hilic_pos_6 <-
    metflow2:::integrateData(object = hilic_pos_5, method = "subject.mean")
)

qc_rsd <- calRSD(object = hilic_pos_6, slot = "QC")
...
###remove some samples (outliers)
(hilic_neg_3 <-
    filterSample(object = hilic_neg_2, min.fraction.peak = 0.5))
...
(hilic_neg_5 <-
    normalizeData(object = hilic_neg_4, method = "mean"))
...
(
  hilic_neg_6 <-
    metflow2:::integrateData(object = hilic_neg_5, method = "subject.mean")
)

qc_rsd <- calRSD(object = hilic_neg_6, slot = "QC")
