...
##get the confounders
age_dis <- sample_info$Age[match(sample_data_dis$sample_id, sample_info$sample_id)]
bmi_dis <- sample_info$bmi[match(sample_data_dis$sample_id, sample_info$sample_id)]
parity_dis <- sample_info$parity[match(sample_data_dis$sample_id, sample_info$sample_id)]
ethnicity_dis <- sample_info$ethnicity[match(sample_data_dis$sample_id, sample_info$sample_id)]
ethnicity_dis <- 
  case_when(
    ethnicity_dis == "Asian" ~ 0,
    ethnicity_dis == "Black" ~ 1,
    ethnicity_dis == "Latina" ~ 2,
    ethnicity_dis == "White" ~ 3
  )


age_val <- sample_info$Age[match(sample_data_val$sample_id, sample_info$sample_id)]
bmi_val <- sample_info$bmi[match(sample_data_val$sample_id, sample_info$sample_id)]
parity_val <- sample_info$parity[match(sample_data_val$sample_id, sample_info$sample_id)]
ethnicity_val <- sample_info$ethnicity[match(sample_data_val$sample_id, sample_info$sample_id)]
ethnicity_val <- 
  case_when(
    ethnicity_val == "Asian" ~ 0,
    ethnicity_val == "Black" ~ 1,
    ethnicity_val == "Latina" ~ 2,
    ethnicity_val == "White" ~ 3,
    ethnicity_val == "Other" ~ 4,
    ethnicity_val == "Caucasian" ~ 5,
    ethnicity_val == "Pacific Islander" ~ 6
  )
...
sample_data_dis_x_rf <- 
  ...
  data.frame(., age = age_dis, bmi = bmi_dis, parity = parity_dis, ethnicity = ethnicity_dis, stringsAsFactors = FALSE) %>% 
  as.matrix()
...
sample_data_val_x_rf <- 
  ...
  as_tibble() %>% 
  dplyr::select(one_of(marker_rf$name)) %>% 
  data.frame(., age = age_val, bmi = bmi_val, parity = parity_val, ethnicity = ethnicity_val, stringsAsFactors = FALSE) %>% 
  as.matrix()
