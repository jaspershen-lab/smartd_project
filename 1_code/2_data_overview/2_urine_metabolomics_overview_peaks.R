...
dir.create(
  "3_data_analysis/2_data_overview/2_urine_metabolomics_overview_peaks",
  recursive = TRUE
)
setwd("3_data_analysis/2_data_overview/2_urine_metabolomics_overview_peaks")
...
plot <-
  ...
  na.value = "#C71000FF") +
  facet_wrap(~ subject_id, ncol = 6, scales = "free")
...
x <-
  ...
  left_join(temp_data@sample_info[, c("sample_id", "subject_id", "GA")], by = c("sample_id")) %>%
  dplyr::mutate(y = 0)
